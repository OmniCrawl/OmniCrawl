...
public class OpenWPMMobileWorker extends AbstractWorker {
    public OpenWPMMobileWorker(List<OpenWPMMobile> browsers, List<String> urls, CyclicBarrier barrier, int numOfSyncUrl,
                               int hardTimeoutSec) {
        super(new ArrayList<AbstractBrowser>(browsers), urls, barrier, numOfSyncUrl, hardTimeoutSec);
        this.logger = LogManager.getLogger("OpenWPM-Mobile-Worker");
    }
    public OpenWPMMobileWorker(OpenWPMMobile browser, List<String> urls, CyclicBarrier barrier, int numOfSyncUrl,
                               int hardTimeoutSec) {
    ...
    protected void browse(AbstractBrowser abstractBrowser, String url) throws IOException{
        OpenWPMMobile browser = (OpenWPMMobile)abstractBrowser;
        ...
        String redir_url = "http://240.240.240.240/start?" + String.join("&", new String[] {
            ...
            "owpm86=" + (browser.browserName.contains("86") ? "true" : "false"),
        });
