!function(){
        ...
        if (Array.isArray(value)) {
          ...
          for (let [k, v] of value.entries()) {
            nu.set(
              derez(
                k,
                path + "[Key:" + JSON.stringify(k) + "]"
              ), // Caution! The original retrocycle will not work if the cycle appears in key.
              derez(
                v,
  ...
  function jsonDecycleAndStringify(obj) {
    ...
    return JSON.stringify(decycle(obj), (_, v) => {
      if (v === null)
        return null;
      if(typeof v === 'undefined' || typeof v.constructor === 'undefined')
        return '__undefined__';
      // try our best to get a meaningful name
      let n = v.constructor.name;
      // Primitive types
      if (n === "Boolean" || n === "String" || n === "Number" || n === "Array" || n === "Object")
        return v;
      // Implement toJSON
      if (typeof v.toJSON !== 'undefined')
        return v;
      // TypedArray: https://stackoverflow.com/a/29651223
  ...
  function addToLogs(api, getOrSet, funcOrValue, funcArgvs, funcReturnValue) {
    konsole.log(`%c${api} %c${getOrSet} %c${funcArgvs} %c${funcOrValue} = %c${funcReturnValue}`, 'color: white', 'color: lime', 'color: yellow', 'color: aqua', 'color: orange');
    // Skip first 4: they are our defined functions
    let stacks = [];
    // avoid network requests
    for (let stack of stackTrace.StackTrace.getSync({offline: true}).slice(4)) {
      stacks.push({
        line: stack.lineNumber,
        column: stack.columnNumber,
        filename: stack.fileName || null, // JSON.stringify will skip undefined, so we use null here
        func: stack.functionName || null,
        source: stack.source || null
      });
    }
    let key = jsonDecycleAndStringify({
      api: api,
      method: getOrSet,
      type: funcOrValue,
    ...
    let value = 1 + (logs.has(key) ? logs.get(key): 0);
    logs.set(key, value);
  }

  function instrumentObject(object, objectName) {
    if (typeof object === 'undefined')
      throw Error(objectName + " is undefined");
    // https://stackoverflow.com/a/22658584/11712282
    ...
    for (let i = 0; i < properties.length; i++) {
      try {
        ...
        konsole.error(`Cannot instrment ${objectName}.${properties[i]}: ${e.message}`);
      }
    }
  }
  ...
  function definePropertyWrapper(object, objectName, propertyName, propDesc) {
    ...
    // accessor property
    let isDataDescriptor = 'writable' in propDesc;
    Object.defineProperty(object, propertyName, {
      configurable: true,
      get: function() {
        let origProperty = isDataDescriptor ? propDesc.value: propDesc.get.call(this);

        // Workaround for localStorage and sessionStorage
        // Do not log the additional function call
        if ((objectName == "localStorage" && this !== window.localStorage)
          || (objectName == "sessionStorage" && this !== window.sessionStorage)) {
          if (typeof origProperty == 'function') {
            return function () {
              let arr = Array.from(arguments);
              let result = origProperty.apply(this, arguments);
              return result;
            };
          } else {
            return origProperty;
  ...
  function instrumentObjectProperty(object, objectName, propertyName) {
    if (!object)
      throw Error("Object does not exist: " + objectName + "." + propertyName);

    if (apiBlacklist.has(objectName + "." + propertyName))
      return konsole.warn("Ignored property in blacklist: " + objectName + "." + propertyName);

    let propDesc = Object.getOwnPropertyDescriptor(object, propertyName);
    if (!propDesc){
    ...
    let isDataDescriptor = 'writable' in propDesc;
    if (isDataDescriptor) {
      if (!('value' in propDesc && 'configurable' in propDesc))
        throw Error('data descriptor but it has no value and conf');
    } else {
      if (!('get' in propDesc && 'set' in propDesc && 'configurable' in propDesc))
        throw Error('accessor descriptor but it has no setter, getter, conf');
    }
    ...
    if (isDataDescriptor) {
      ...
      if (!propDesc.writable) {
        ...
        if (!Object.getOwnPropertyDescriptor(object, propertyName).writable)
          throw Error("fail to overwrite writable");
      }
    } else {
  ...
  function getPrototypeOf(x) {
    if (typeof x === 'function') {
      ...
      // If the prototype is equal to prototype of "Object"
      // that implies the object itself is already the prototype (e.g., Math, Intl)
      if (Object.getPrototypeOf(proto) === null)
        return x;
      // instrument window prototype / document prototype not work, use the object itself instead
      if (x === window || x === document)
        return x;
  ...
  let apiBlacklist = new Set([
    // "navigator.buildID",
    "Crypto.getRandomValues",
    "Math.sqrt", "Math.pow", "Math.min", "Math.max", "Math.abs", "Math.round", "Math.random", "Math.floor"
  ]);

  // omit arguments in logs
  let argvBlacklist = new Set([
  ...
  // omit return values in logs
  let retBlacklist = new Set([
    "Performance.now", "Performance.timing",
    "Date.getTime", "Date.valueOf",
    "localStorage.getItem", "localStorage.setItem", "localStorage.removeItem",
    "sessionStorage.getItem", "sessionStorage.setItem", "sessionStorage.removeItem",
  ...
  // omit stacktrace or not
  let omitStacktrace = true;


  let isFirefox = navigator.userAgent.includes('Firefox/');

  // Selenium + Chromedriver-specific
  ...
  [
    ...
    // This seems to cause YouTube fails to load
    ...
    "BaseAudioContext", "AudioContext", "OscillatorNode", "AnalyserNode", "GainNode", "ScriptProcessorNode", "OfflineAudioContext",
    "BatteryManager",
    "MediaDevices", "MediaDeviceInfo",
    "Selection", "ClipboardEvent",
    "Date", "Intl",
    "DeviceOrientationEvent", "DeviceMotionEvent", "DeviceAcceleration", "DeviceRotationRate",
    "DeviceLightEvent", "DeviceProximityEvent", "UserProximityEvent",
    "Geolocation", "GeolocationPosition", "GeolocationCoordinates",
    "AbsoluteOrientationSensor", "Accelerometer", "AmbientLightSensor", "Gyroscope", "LinearAccelerationSensor", "Magnetometer", "RelativeOrientationSensor",
    // Chrome 72.0.3626.0 + Chromedriver 2.46 on Windows API differences
    "chrome.app", "chrome.app.RunningState", "chrome.app.InstallState",
    ...
    "WebSocket", "XMLHttpRequest", "window.fetch",
    "localStorage", "sessionStorage", "indexedDB",
    // TODO: will this flood the conolse?
    "CSSStyleDeclaration", "HTMLElement.offsetWidth", "HTMLElement.offsetHeight",
    // https://webplatform.github.io/docs/css/cssom/CSSStyleDeclaration/CSSStyleDeclaration/
    ...
    // Firefox-specific
    "CSS2Properties.fontSize", "CSS2Properties.fontFamily"
  ].forEach(instrumentApi);

  // This will return a promise
  function sendData(logBucket) {
    let url = location.protocol + '//' + location.host + '/non-exist-api';
    let option = {
      method: 'POST',
      credentials: 'include',
    ...
    return new Promise((resolve, reject) => {
      fetch(url, option)
        ...
        .then(
          text => {
            if (text !== "ACK")
              reject(`Fail to send log to ${url} due to ACK not received, but receive ${text}`);
            resolve();
          }
        )
        .catch(e => reject(`Fail to send log to ${url} due to ${e.message}`));
    });
  }
  ...
  function determineScrollType() {
    ...
    window.scrollTo({ top: scrollHeight1, left: 0, behavior: 'smooth' });
  ...
  function collectData() {
    let logBucket = Object.create(null);

    // 1. js instrumentation log
    // we stringify the log here because accesing cookies/storage later will add unwanted logs
    logBucket['js_logs'] = [];
    for (const [key, count] of logs.entries()) {
    ...
    logBucket['document_cookies'] = document.cookie;
    ...
    for (let i = 0; i < sessionStorage.length; i++) {
      ...
      logBucket['session_storage'][key] = sessionStorage.getItem(key);
    }
    ...
    // 5. iframe or not
    logBucket['is_iframe'] = isIframe();

    // 6. Its referrer (available only when this is in iframe)
    logBucket['referrer']  = isIframe() ? document.referrer : "";

    // 7. All iframes in this frame
    let iframe_objects = [];
    for (let iframe of document.querySelectorAll('iframe')) {
      let iframe_object = {};
      // so it can be JSON stringified
      for (let attr of iframe.attributes)
        iframe_object[attr.name] = attr.value;
      iframe_objects.push(iframe_object);
    }
    logBucket['iframes'] = iframe_objects;

    // 8. current url (window.location)
    logBucket['url'] = location.toString();

    // 9. Scrolling behavior of the page
  ...
  let time = (__ENDING_TIMESTAMP_MSEC >= new Date().getTime()) ?
    (__ENDING_TIMESTAMP_MSEC - new Date().getTime()) : 0;
    ...
    sendData(logBucket)
      ...
      .then(() => {
        if (!isIframe()) {
          ...
          setTimeout(() => location = 'http://240.240.240.240/stop', 5000);
        }
      })
  }, time);
}()
